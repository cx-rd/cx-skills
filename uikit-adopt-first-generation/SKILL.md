...
4. 不得把 custom UI 當成預設路徑。
5. 只有在 UI-kit 沒有對應能力，或現有 extension point 明確不足時，才允許建立 app-local UI。

## 執行流程

1. 先判斷需求是 page-level、section-level 還是 component-level。
2. 先做 UI-kit capability audit。
3. 若屬於完整畫面 / 頁面，繼續讀 `init-upgrade-migration-skills/AI_Skill_Generation_Spec.md`，並委派給對應 specialized skill 或 `generate-app-orchestrator`。
4. 若屬於局部區塊或單一元件，仍必須維持 `adopt-first`，不得繞過 UI-kit 直接手刻近似版。
